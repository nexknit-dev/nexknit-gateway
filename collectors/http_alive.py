...
class HttpAliveCollector(BaseCollector):
    ...
    def collect(self) -> Dict[str, Any]:
        """Collect status for all URLs - each URL has independent status using threads"""
        results = {}
        all_failed = True
        thread_results = {}
        
        def check_url_thread(idx: int, url: str):
            """Thread function to check URL status"""
            status, latency_ms = self._check_url_single(url)
            thread_results[idx] = (url, status, latency_ms)
        
        # Start threads for all URLs
        threads = []
        for idx, url in enumerate(self._urls):
            thread = threading.Thread(target=check_url_thread, args=(idx, url))
            thread.daemon = True
        ...
        # Process results
        for idx in range(len(self._urls)):
            if idx in thread_results:
                url, status, latency_ms = thread_results[idx]
                ...
                domain = self._extract_domain(url)
                results[f"http_{idx}_status"] = f"{domain}\n⚠️ Timeout\nRequest timed out"
                print(f"[WARN] HTTP Alive Collector: Timeout checking {url}")
                self._check_alert(url, 0)

        return results

    def _check_url_single(self, url: str) -> tuple:
        """Check status of single URL (thread-safe)"""
        try:
            start = time.time()
